...
def make_mongo_connection(collection_name):
    ...
    client = MongoClient(mongo_uri, ssl=ssl_required, replicaSet='Cluster0-shard-0')
...
def bulk_mongo_insert(mongo_colln, obj_list):
...
def bulk_mongo_update(mongo_colln, obj_list):
    ...
    try:
        for new_obj in obj_list:
            mongo_colln.update_one({'checksum': new_obj['checksum']}, {"$set": new_obj},
                                   upsert=True)
    except:
        logging.error("Mongo Insert Exception.")
        raise
